"""
Export RF-DETR Nano (object detection) to CoreML-backed ExecuTorch .pte

Requirements:
    pip install rfdetr executorch coremltools torchao

Usage:
    python export_rfdetr_detection_coreml.py
    python export_rfdetr_detection_coreml.py --output my_model.pte
    python export_rfdetr_detection_coreml.py --compute-unit cpu_and_ne

Output tensor layout (3 tensors):
    [0] boxes_xyxy     [Q, 4]   float32  xyxy in pixel coords (0..resolution)
    [1] scores         [Q]      float32  sigmoid confidence in [0, 1]
    [2] labels         [Q]      float32  COCO class IDs

Default settings (best from autoresearch exp-109 + ALL compute unit):
    model      : RFDETRNano
    resolution : 384px  (native)
...
class RFDETRDetectionWrapper(nn.Module):
    """Outputs (boxes_xyxy [Q,4], scores [Q], labels [Q])."""
    ...
    def forward(self, x: torch.Tensor):
        ...
        return torch.stack([x1, y1, x2, y2], dim=-1), scores, labels.float()
...
COMPUTE_UNITS = {
    "all": None,        # resolved after coremltools import
    "cpu_and_ne": None,
    "cpu_only": None,
}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export RF-DETR Nano detection to CoreML .pte")
    parser.add_argument("--output", type=str, default="rfdetr_detection_coreml.pte",
